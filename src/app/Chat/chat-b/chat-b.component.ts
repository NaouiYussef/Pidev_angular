...
@Component({
  ...
  styleUrls: ['./chat-b.component.css']
})
export class ChatBComponent implements OnInit {
  ...
  public getChats(): void {
    ...
    const url = `${this.apiServerUrl}/Chat/getChats`; // construct the URL
    this.chatService.getChats(url).subscribe(
      (response: Chat[]) => {
        this.chats = response;
      },
  ...
  connect(s: string): void {
    ...
    if (this.usernamePage && this.chatPage) {
      this.renderer.addClass(this.usernamePage.nativeElement, 'hidden');
      this.renderer.removeClass(this.chatPage.nativeElement, 'hidden');
    }
  ...
  onConnected(): void {
    ...
    this.stompClient.subscribe('/user/' + this.username + '/private', this.onMessageReceived.bind(this));

    // Tell your username to the server
    this.stompClient.send('/app/chat.addUser',
      {},
      JSON.stringify({sender: this.username, receiver: 'admin', type: 'JOIN'})
    );
